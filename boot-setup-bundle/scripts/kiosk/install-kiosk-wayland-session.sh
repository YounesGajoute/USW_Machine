...
# (from boot-setup-bundle: US_MACHINE_ROOT is set by install-techmac-boot.sh)
#
...
if [[ ! -f "${PROJECT_ROOT}/package.json" ]]; then
	echo "Set US_MACHINE_ROOT to the US Machine repo root, or run boot-setup-bundle/apply.sh." >&2
	exit 1
fi
