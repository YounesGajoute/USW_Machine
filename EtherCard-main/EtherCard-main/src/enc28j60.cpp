...
void ENC28J60::initSPI () {
    ...
    // 4 MHz @ 16 MHz AVR clock (prescaler /4, SPI2X=0).
    // Reduced from 8 MHz to lower SPI bus switching noise, which can couple
    // into the ENC28J60 TX analog front-end and cause intermittent frame loss.
    SPCR = bit(SPE) | bit(MSTR); // SPR1=0, SPR0=0 → prescaler /4
    bitClear(SPSR, SPI2X);        // SPI2X=0 → final freq = 16 MHz / 4 = 4 MHz
}
...
static uint16_t readPhyByte (byte address) {
    ...
    // Per ENC28J60 spec, both MII bytes must be read to complete the transaction.
    uint16_t lo = readRegByte(MIRD);
    uint16_t hi = readRegByte(MIRD+1);
    return (hi << 8) | lo;
}
...
byte ENC28J60::initialize (uint16_t size, const byte* macaddr, byte csPin) {
    ...
    // ENC28J60 errata B7 issue 5: ERXRDPT must be initialised to an ODD value.
    // Using RXSTART_INIT (0x0000, even) silently disables the receive buffer.
    // Fix: initialise to RXSTOP_INIT (0x0BFF, odd).
    ...
    // Accept only unicast-to-mymac and broadcast, both with valid CRC.
    // PMEN (pattern match) is removed: it is redundant with UCEN+BCEN and
    // could accept extraneous frames from other LAN devices, flooding the
    // 3 KB receive ring and starving our own traffic.
    writeRegByte(ERXFCON, ERXFCON_UCEN|ERXFCON_CRCEN|ERXFCON_BCEN);
...
bool ENC28J60::isLinkUp() {
    // readPhyByte() now returns the full 16-bit PHY register value.
    // PHSTAT2.LSTAT is bit 10 of the 16-bit register.
    return (readPhyByte(PHSTAT2) >> 10) & 1;
}
...
void ENC28J60::packetSend(uint16_t len) {
    ...
    resume_last_transmission:
        ...
        // ENC28J60 errata B7 / issue 13: TXIF is asserted by the MAC before the
        // PHY has finished driving bits onto the wire.  Any SPI access that
        // occurs after TXIF fires but before the PHY is truly done silently
        // corrupts the outgoing frame.
        //
        // Root cause of the polling-loop approach: every readRegByte(EIR) inside
        // the poll loop hits the SPI bus during the live PHY window, which can
        // corrupt an ongoing transmission.
        //
        // Fix: do NOT poll at all.  Issue a single worst-case blocking delay
        // immediately after TXRTS is set.  At 10 Mbps, the longest legal Ethernet
        // frame (1500-byte payload) takes:
        //   8 B preamble/SFD + 14 B Eth + 1500 B payload + 4 B FCS = 1526 B
        //   → 1526 × 8 bits / 10 Mbps ≈ 1.22 ms
        // Adding a 1 ms safety margin → 2.25 ms total.
        // Round up to 3 ms so that no SPI operation can ever hit the live window.
        delay(10); // diagnostic: 10 ms >> worst-case ~1.22 ms

        // Now the PHY is guaranteed idle; check outcome exactly once.
        {
            uint8_t eir = readRegByte(EIR);
            if (!(eir & EIR_TXERIF)) {
