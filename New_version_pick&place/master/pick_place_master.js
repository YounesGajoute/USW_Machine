...
export function registerPickPlaceConfigStore(store) {
...
export function loadPickPlaceConfig() {
  if (externalConfigStore?.load) {
    try {
      configCached = validatePickPlaceConfig(externalConfigStore.load())
      return { ...configCached }
    } catch (err) {
      console.warn(`[pick-place] config load failed (external store): ${err.message}`)
    }
  }
  try {
...
export function savePickPlaceConfig(partial) {
  ...
  if (externalConfigStore?.save) {
    externalConfigStore.save(next)
    configCached = next
    return { ...configCached }
  }
...
export function getConfigPath() {
  if (externalConfigStore?.path) return externalConfigStore.path()
  return CONFIG_PATH
}
...
export async function handlePickPlaceHttpRequest(req, res, { apiPort = API_PORT } = {}) {
  ...
  try {
    ...
    if (req.method === 'GET' && routePath === '/api/pick-place/config') {
      let live = {}
      try {
        live = await fetchConfig()
      } catch {
        // File-backed config is still valid when the controller is offline (settings UI).
      }
      apiSendJson(res, 200, {
        ok: true,
        config: getPickPlaceConfig(),
        path: getConfigPath(),
        connected: isConnected(),
        ...live,
      })
    ...
    if (req.method === 'POST' && routePath === '/api/pick-place/move_a_t2') {
      ...
      const position = body.position ?? body.distanceMm ?? 0
      const speed = body.speed ?? cfg.movementSpeedMmS
      apiSendJson(res, 200, await moveAmmT2(Number(position), Number(speed)))
      return true
    }
