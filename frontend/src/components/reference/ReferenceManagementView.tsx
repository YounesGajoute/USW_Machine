...
export interface ReferenceManagementViewProps {
  title?: string
  /** Label for the primary name field (default: Reference). */
  nameLabel?: string
  /** Singular resource name for dialogs (default: Reference). */
  resourceSingular?: string
  /** Force uppercase on name input (default: true for barcodes). */
  ...
  /** Virtual keyboard mode per form field key (includes built-in `name` / `description`). */
  keyboardFieldConfig?: Record<string, KeyboardFieldConfig>
  /** Return an error message to block save, or null when valid. */
  validateForm?: (form: Record<string, unknown>, mode: 'create' | 'edit') => string | null
  /** Hide the optional description field in create/edit dialogs. */
  hideDescriptionField?: boolean
  /** Hide the search bar above the table. */
  hideSearch?: boolean
...
export function ReferenceManagementView({
  ...
  const patchForm = (patch: Record<string, any>) => setForm(f => ({ ...f, ...patch }))

  const handleSave = async () => {
    if (!form.name?.trim()) { setFormError(`${nameLabel} is required`); return }
    const validationError = validateForm?.(form, showCreate ? 'create' : 'edit')
  ...
  const kbAppend = (ch: string) => {
    if (!kbTarget) return
    const config = keyboardFieldConfig[kbTarget]
    if (config?.decimalInput) {
      if (ch === '.' && String(form[kbTarget] ?? '').includes('.')) return
      if (ch !== '.' && !/^\d$/.test(ch)) return
    }
    setFormField(kbTarget, ((form[kbTarget] as string) ?? '') + ch)
  }
  ...
  const activeKbConfig = useMemo((): KeyboardFieldConfig | null => {
    if (!kbTarget) return null
    if (keyboardFieldConfig[kbTarget]) return keyboardFieldConfig[kbTarget]!
    if (kbTarget === 'name') return { label: nameLabel }
    if (kbTarget === 'description') return { label: 'Description' }
    return null
    ...
    <div style={{ height: '100%', backgroundColor: colors.background, padding: '20px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <Dialog open={deleteId !== null} onOpenChange={(open) => { if (!open) { setDeleteId(null); setDeleteError(null) } }}>
        <DialogContent style={{ width: KIOSK_DLG_CONFIRM_W, maxWidth: '100%', maxHeight: KIOSK_DLG_MAX_H }}>
          <DialogHeader>
            <DialogTitle>Delete {resourceSingular}</DialogTitle>
            <DialogDescription>This action cannot be undone.</DialogDescription>
          </DialogHeader>
          <div style={{ marginBottom: '20px' }}>
            <p style={{ color: colors.text, fontSize: '16px', marginBottom: '10px' }}>
              Are you sure you want to delete this {resourceSingular.toLowerCase()}?
            </p>
            {deleteTarget && (
              <div style={{
