...
export function LoadedReferenceInfo({ reference, shrinkTubes = [] }: LoadedReferenceInfoProps) {
  const { colors } = useTheme()
  ...
  const hasShrinkTube = referenceHasShrinkTube(reference)
  const shrinkTube = hasShrinkTube
    ? shrinkTubes.find(t => t.id === reference.shrink_tube_id)
    : undefined
    ...
    >
      <div style={{ minWidth: 0, flexShrink: 0 }}>
        ...
        {!hasShrinkTube ? (
          ...
          >
            No shrink tube — assign in References before production.
          </p>
        ) : (
          <p
            style={{
            ...
            title={shrinkTube ? formatShrinkTubeLabel(shrinkTube) : String(reference.shrink_tube_id)}
          >
            Tube: {shrinkTube ? formatShrinkTubeLabel(shrinkTube) : reference.shrink_tube_id}
          </p>
        )}
      </div>
