...
export default function DialogVirtualKeyboard({
  ...
  const [isNumbers, setIsNumbers] = useState(numericOnly)
  ...
  const handleKeyClick = useCallback((key: string) => {
    ...
    if (isShift && !isNumbers && !isCapsLock && !numericOnly) {
      onKeyPress(key.toUpperCase())
      setIsShift(false)
    } else if (isCapsLock && !isNumbers && !numericOnly && key.match(/[a-z]/)) {
      onKeyPress(key.toUpperCase())
    } else {
      onKeyPress(key)
    }
  }, [numericOnly, isShift, isNumbers, isCapsLock, onKeyPress])
  ...
  const currentLayout = useMemo(
    () => numericOnly ? numericOnlyLayout : (isNumbers ? numbersLayout : qwertyLayout),
    [numericOnly, isNumbers, numericOnlyLayout, numbersLayout, qwertyLayout]
  )

  const shouldUpperCase = useCallback((key: string) => {
    if (numericOnly || isNumbers) return false
    if (isCapsLock && key.match(/[a-z]/)) return true
    if (isShift && key.match(/[a-z]/)) return true
    return false
  }, [numericOnly, isNumbers, isCapsLock, isShift])
