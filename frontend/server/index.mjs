...
import { mergePickPlaceConfigPatch, normalizePickPlaceConfig } from '../../backend/lib/pickPlaceConfigStore.mjs'
...
function normalizeCentringMechanism(value) {
  ...
  if (raw === 'upper_and_lower_centring_mechanism' || raw === 'upper_and_lower' || raw === 'both') {
...
app.patch('/api/shrink-tubes/:id', optionalAuth, (req, res) => {
  ...
  try {
    ...
    const now = new Date().toISOString()
    if (name !== undefined) {
      const conflict = db.prepare('SELECT id FROM shrink_tubes WHERE LOWER(name) = LOWER(?) AND id != ?').get(String(name).trim(), id)
    ...
    if (diameter_mm !== undefined) {
      db.prepare('UPDATE shrink_tubes SET diameter_mm = ?, updated_at = ? WHERE id = ?').run(parsePositiveNumber(diameter_mm, 'Diameter'), now, id)
    }
    if (length_mm !== undefined) {
      db.prepare('UPDATE shrink_tubes SET length_mm = ?, updated_at = ? WHERE id = ?').run(parsePositiveNumber(length_mm, 'Length'), now, id)
    }
    ...
    if (rbk !== undefined) db.prepare('UPDATE shrink_tubes SET rbk = ?, updated_at = ? WHERE id = ?').run(normalizeRbk(rbk), now, id)
    if (is_active !== undefined) db.prepare('UPDATE shrink_tubes SET is_active = ?, updated_at = ? WHERE id = ?').run(is_active ? 1 : 0, now, id)
    res.json(mapShrinkTubeRow(db.prepare('SELECT * FROM shrink_tubes WHERE id = ?').get(id)))
  } catch (err) {
    res.status(400).json({ message: err.message || 'Invalid shrink tube data' })
  }
})
...
app.delete('/api/shrink-tubes/:id', optionalAuth, (req, res) => {
  const { id } = req.params
  if (!db.prepare('SELECT * FROM shrink_tubes WHERE id = ?').get(id)) {
    return res.status(404).json({ message: 'Shrink tube not found' })
  }
  if (db.prepare('SELECT id FROM product_references WHERE shrink_tube_id = ? LIMIT 1').get(id)) {
...
app.post('/api/references', optionalAuth, (req, res) => {
  ...
  db.prepare(`
    INSERT INTO product_references (
      ...
      rbk, shrink_tube_id, created_at, updated_at
    )
    VALUES (?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    id,
