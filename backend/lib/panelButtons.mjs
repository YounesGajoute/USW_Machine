...
async function pollOnce(ecm) {
  ...
  if (initRising && !_initLock) {
    ...
    if (
      status.referenceLoaded
      && !status.initialized
      && !status.initInProgress
      && getMachineInitBlockReason() == null
    ) {
      _initLock = true
  ...
  if (startRising && !_productionLock) {
    if (canEnqueueProduction()) {
      _productionLock = true
      console.log('[PanelButtons] DI1 START_BUTTON — enqueue production job')
      try {
